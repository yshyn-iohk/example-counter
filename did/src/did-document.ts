...
/** DID URL schema */
export const DIDURLSchema = z.string()
  .check(
    z.startsWith('did:'),
    z.minLength(5),
...
export const DIDSchema = z.string()
  .check(
    z.startsWith('did:'),
    z.minLength(5),
    z.refine(val => val.split(':').length >= 3 && !/[\/?#]/.test(val), 'Invalid DID format'),
  )
  .brand('DID');
export type DID = z.infer<typeof DIDSchema>;

/** Multibase public key */
export const PublicKeyMultibaseSchema = z.string()
  .check(
    z.minLength(2),
    z.regex(/^[0-9A-Za-z]/),
...
/** Parsing Helpers */
export const parseDIDDocument = (input: unknown) => DIDDocumentSchema.parse(input);
export const parseDIDURL = (input: unknown) => DIDURLSchema.parse(input);
export const parseDID = (input: unknown) => DIDSchema.parse(input);
export const parseVerificationMethod = (input: unknown) => VerificationMethodSchema.parse(input);
export const parseService = (input: unknown) => ServiceSchema.parse(input);
export const parseDIDResolutionResult = (input: unknown) => DIDResolutionResultSchema.parse(input);
